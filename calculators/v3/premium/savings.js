/* retiresave3.m Jack DeJong's program for bootstrapping retirement savings results:
...
var rs  = require('../../../lib/app/v3/premium/savings');
...
module.exports = function(value) {
  ...
  // [2] 1        'For annual contribution select (1) % of salary/wages or (2) dollar amount',...
  // [3] 10       'What is your annual contribution as % of your salary/wages?',...
  // [4] 12500    'What is your annual contribution as a fixed dollar amount?',...
  // [5] 3        'Select % by which you will increase your contributions each year',...
  // [6] 1        'Estimate your annual investment expenses in percentage',...
  // [7] 1250000  'What is your current retirement portfolio value?'

  // --- Value 2 defaults
  // [0] 50       'What percentage of your retirement portfolio is allocated to stocks?',...
  // [1] 50       'What percentage of your portfolio should be allocated to stocks at retirement?',...
  // [2] 40       'What percentage of your retirement portfolio is allocated to bonds?',...
  // [3] 30       'What percentage of your stocks is allocated to small/midcap equities?',...
  // [4] 25       'What percentage of your stocks is allocated to international equities?',...
  // [5] 3        'For bonds select (1) 10 year Treasury bonds (2) US Corporate bonds or (3) CDs',...
  // [6] 2        'What annual fixed percentage rate do you earn on your CDs?',...
  // [7] 2        'For cash select (1) 1 year Treasury bills or (2) Money market fund',...
  // [8] 0        'What annual fixed percentage rate do you earn on your Money market fund?'

  var value1 = value.slice(0, 8),
      value2 = value.slice(8);
  ...
  rs.readFile(filepath, function(err, _results) {
    ...
    // Set critical inputs:
    var inflation = Number(Big(value1[5]).div(100).toFixed(15)), //input the annual cost of living adjustment for inflation.
        expense = Number(Big(value1[6]).div(100).toFixed(15)), //input the annual investment expenses as given by the retiree.
        originalbondwt = Number(Big(value2[2]).div(100).toFixed(15)), //input the portion of the portfolio that is invested in bonds.
        stockwt = Big(value2[0]).div(100), //input the portion of the portfolio that is invested in stocks.
        stockwtretire = Number(Big(value2[1]).div(100).toFixed(15)), //input the portion of the portfolio that is invested in stocks at retirement.
        // TODO: Check if this should default to 0.45 (45%);
        sp500wt = Number(stockwt.times((Big(100 - value2[3] - value2[4]).div(100))).toFixed(15)), //45% or input amount of stock portion is invested in S&P 500.
        russwt = Number(stockwt.times(Big(value2[3]).div(100)).toFixed(15)), //30% or input amount of stock portion is invested in Russell 2000.
        eafewt = Number(stockwt.times(Big(value2[4]).div(100)).toFixed(15)), //25% or input amount of stock portion is invested in MSCI EAFE Index.
        ...
        // %compute initial annual contribution amount using either percent of salary
        // %or fixed dollar amount as chosen.
        if (value1[2] === 2) { // %selected fixed dollar amount to contribute.
          originalmoinvestamt = Number(Big(value1[4]).div(12)); // %convert annual to monthly.
        }
        else { // %selected 1 for contributing percentage of salary as annual amount
          originalmoinvestamt = Number(Big(value1[3]).div(100).times(Big(value1[1]).div(12).toFixed(15))); //set to contribute retirees target initial annual amount.
        }

    var years = value1[0], //years = number of years to retirement as inputted by retiree.
        ...
        originalbal = value1[7], //set originbal to the current value in the retirement portfolio.
    ...
    for (var i = 0; i < 1056; i++) {
      monthlydata1970to2013[i][8] = Number(Big(value2[6]).div(1200)); //%converts input CD annual rate to a monthly rate
      monthlydata1970to2013[i][9] = Number(Big(value2[8]).div(1200)); //%converts input MMF annual rate to a monthly rate
    }

    // %Assign which bond investment will be used based on inputs:
    var bf;
    if (value2[5] == 2) { // %selected Merrill Lynch US Corporate Bond Index
      bf = 3; //ML US Corporate Bond returns in column 4 (index 3)
    }
    else if (value2[5] == 3) { // %selected CD with user inputted fixed rate
    ...
    var cf;
    if (value2[7] == 2) { // %selected MMF with user inputted fixed rate
    ...
    for (var bs = 0; bs < nboot; bs++) { // %nboot is the number of bootstrap samples we want.
      ...
      for (var mn = 0; mn < horizon; mn++) { // %maximum horizon considered is 50 years or 600 months, but it can be shorter as inputted.
        ...
        if (mn % 12 === 11) { // %identified 12th month of year so update inflation and deduct fees for the year.
          ...
          sp500wt = Number(_stockwt.times((Big(100 - value2[3] - value2[4]).div(100))).toFixed(15)); // %fixed portion of stocks is invested in S&P 500.
          russwt = Number(_stockwt.times(Big(value2[3]).div(100))); // %fixed portion of stocks is invested in Russell 2000.
          eafewt = Number(_stockwt.times(Big(value2[4]).div(100))); // %fixed portion of stocks is invested in MSCI EAFE Index.
    ...
    accumtable['10% Value at Risk']       = quantile(results, 0.1).toExponential();
    accumtable['5% Value at Risk']        = quantile(results, 0.05).toExponential();
    accumtable['1% Value at Risk']        = quantile(results, 0.01).toExponential();
    accumtable['Lowest Balance']          = results[0].toExponential();


    keytable['Mean Balance']                            = mean(results).toExponential();
